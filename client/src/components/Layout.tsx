...
const Layout: React.FC<LayoutProps> = ({ children }) => {
  ...
  const navLinks = token ? loggedInNavLinks : loggedOutNavLinks;
            ...
            <nav className="hidden md:flex items-center space-x-10">
              {navLinks.map((link) => (
                <Link
                  key={link.path}
                  to={link.path}
                  className={`font-body text-m font-medium transition-colors duration-300 ${location.pathname === link.path
            ...
            <div className="px-4 py-4 space-y-4">
              {navLinks.map((link) => (
                <Link
                  key={link.path}
                  to={link.path}
                  className={`block py-2 font-body text-base ${location.pathname === link.path
